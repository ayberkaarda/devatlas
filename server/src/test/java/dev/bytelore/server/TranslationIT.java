...
class TranslationIT extends ContentApiTestSupport {
  ...
  /**
   * A translated body crosses the same write boundary a canonical body does, and is subject to the
   * same guarantee: what the translator wrote is what is stored. The fixture deliberately mixes
   * Turkish diacritics with the punctuation an HTML serializer rewrites, because a translation is
   * the one body where a silent character substitution is hardest for a reviewer to spot.
   */
  @Test
  void aTranslatedBodyRoundTripsByteForByte() throws Exception {
    String token = editorToken();
    UUID trackId = createTrack(token);
    UUID moduleId = createModule(token, trackId);
    UUID lessonId = createLesson(token, moduleId);
    ...
    String body =
        """
        # Degerleri karsilastirma

        `==` yerine `===` kullanin; a < b ve b > a ikisi de gecerli.

        ```ts
        const cevap: number = 42;
        ```

        > Alinti: "olcun" -- farkliysa bana@ornek.com adresine yazin.
        """;

    TranslationUpsertRequest request = new TranslationUpsertRequest("Baslik", body, null);
    MvcResult created =
        mockMvc
            .perform(
                put("/api/v1/admin/translations/LESSON/" + lessonId + "/tr")
    ...
    assertThat(json(created).path("body").asString()).isEqualTo(body);

    MvcResult read =
        mockMvc
            .perform(
                get("/api/v1/admin/translations/LESSON/" + lessonId)
                    .header(HttpHeaders.AUTHORIZATION, "Bearer " + token))
            .andReturn();
    assertThat(read.getResponse().getStatus()).isEqualTo(200);
    assertThat(json(read).path("translations").get(0).path("body").asString()).isEqualTo(body);
  }

  @Test
  void aTranslatedBodyCarryingAScriptTagIsRefused() throws Exception {
    String token = editorToken();
    UUID trackId = createTrack(token);
    UUID moduleId = createModule(token, trackId);
    UUID lessonId = createLesson(token, moduleId);
    ...
    TranslationUpsertRequest request =
        new TranslationUpsertRequest("Baslik", "Merhaba <script>alert(1)</script>", null);
    MvcResult result =
        mockMvc
            .perform(
                put("/api/v1/admin/translations/LESSON/" + lessonId + "/tr")
    ...
    assertThat(json(result).path("message").asString()).contains("<script>");
  }
