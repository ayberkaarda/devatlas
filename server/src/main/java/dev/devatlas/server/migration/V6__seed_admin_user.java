package dev.devatlas.server.migration;
...
public class V6__seed_admin_user extends BaseJavaMigration {
  ...
  public V6__seed_admin_user(
      @Value("${devatlas.seed.admin-email:}") String adminEmail,
      @Value("${devatlas.seed.admin-password:}") String adminPassword,
  ...
  public void migrate(Context context) throws Exception {
    if (adminEmail == null || adminEmail.isBlank()) {
      throw new IllegalStateException(
          "devatlas.seed.admin-email is not set. Supply the administrator's address through the"
              + " environment before the first start.");
    }
    if (adminPassword == null || adminPassword.isBlank()) {
      throw new IllegalStateException(
          "devatlas.seed.admin-password is not set. Supply the initial administrator password"
