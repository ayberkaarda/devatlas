...
import dev.bytelore.server.content.admin.dto.AuditLogItemResponse;
...
import dev.bytelore.server.pipeline.PipelineAuditor;
...
public class AdminBlogPostService {
  ...
  public AdminBlogPostResponse create(CreateBlogPostRequest request, UUID actorUserId) {
    ...
    post.setBodyMarkdown(normalizeAndValidateBody(request.bodyMarkdown()));
  ...
  public PageResponse<AdminBlogPostResponse> list(
    ...
    String titlePattern = buildTitlePattern(query);

    Page<BlogPost> result = blogPosts.search(statusFilter, sourceFilter, titlePattern, pageable);
  ...
  public BlogPost createAutoDraft(
    ...
    post.setBodyMarkdown(normalizeAndValidateBody(bodyMarkdown));
    post.setStatus(BlogStatus.DRAFT);
  ...
  /**
   * Normalizes a post body and admits it, or refuses the write.
   *
   * <p>The returned value is the author's own text: normalization is the only transformation it
   * undergoes, so what was submitted is what is stored and served back. The safety check is a
   * predicate over that text, not a rewrite of it -- a body carrying markup outside the allow-list
   * is refused with the offending elements named, rather than silently altered.
   *
   * <p>A pipeline draft reaches this method too, and passes it: the feed excerpt it embeds has
   * already been cut down to the same allow-list by the transformer, before it was templated.
   */
  ...
  /**
   * Builds the {@code LIKE} pattern for {@link BlogPostRepository#search}'s title filter. A blank
