...
public class AdminMindMapService {
  ...
  private MindMapNodeResponse validate(
    ...
    // The label is stored exactly as it was written, once normalized. A node label is rendered by
    // interpolation and never as markup, so the write boundary asserts that it carries no markup
    // rather than deleting any: stripping tags here would silently rewrite a label an author can
    // still see in their own editor, and it also entity-encoded ordinary characters -- an author
    // writing "x = y" got "x &#61; y" stored under their name.
    String label = TextNormalizer.normalize(node.label());
    if (label == null || label.isBlank()) {
      throw new ApiException(
          ErrorCode.MIND_MAP_INVALID, "Node '%s' has a blank label.".formatted(node.id()));
    }
    MarkdownSanitizer.validatePlainText(label, "label of node '%s'".formatted(node.id()));
