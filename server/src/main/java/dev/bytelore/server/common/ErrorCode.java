...
public enum ErrorCode {
  ...
  /**
   * A markdown body or a plain-text field that would render markup outside the server's allow-list.
   *
   * <p>The write is refused rather than rewritten. Authored content is stored exactly as it was
   * submitted, so the write boundary can only accept or reject -- it cannot quietly delete the
   * offending markup and hand back something the author never wrote. The message names the elements
   * and attributes that were rejected, which is the information needed to fix the submission.
   */
  UNSAFE_HTML(HttpStatus.UNPROCESSABLE_CONTENT),
