 ...
 * in — stored (`downloaded`), not yet fetched (`not-downloaded`), waiting
 * its turn (`queued`), superseded by a newer version (`update`), or stopped
 * with an error (`failed`). They belong in one union rather than a second
 * icon component, so that every state mark in the application is one shape
 * vocabulary. Adding a member makes every lookup keyed by this type — the
 * colour and label tables below, and the shape switch — fail to compile
 * until the new member is handled there too.
 */
export type StateGlyphKind =
  'completed' | 'downloaded' | 'not-downloaded' | 'queued' | 'update' | 'failed';
 ...
 * and `failed` put a mark inside the same ring, `queued` is a ring on its
 * own with a broken outline, and the rest are paths with no ring at all.
 */
interface GlyphShape {
  readonly circle?: { readonly dashArray?: string };
...
function shapeFor(kind: StateGlyphKind): GlyphShape {
  switch (kind) {
    case 'completed':
      ...
      return { circle: {}, paths: ['m5.15 8.2 2 2 3.7-4.4'] };
    case 'downloaded':
      // An arrow coming to rest on a line beneath it. The tray is what
      // distinguishes "this is stored" from "this can be fetched" — the
      // same arrow without it is `not-downloaded`.
      return { paths: ['M8 2.5v7', 'M4.8 7.2 8 10.4l3.2-3.2', 'M3.5 13.2h9'] };
    case 'not-downloaded':
      // The `downloaded` arrow with its tray removed: nothing has landed
      // here yet.
      return { paths: ['M8 2.5v7', 'M4.8 7.2 8 10.4l3.2-3.2'] };
    case 'queued':
      // A ring drawn as dashes instead of a solid line, reading as
      // "waiting its turn" rather than "in progress" or "done".
      return { circle: { dashArray: '2.2 2.1' }, paths: [] };
    case 'update':
    ...
    case 'failed':
      ...
      return { circle: {}, paths: ['M5.8 5.8l4.4 4.4', 'M10.2 5.8l-4.4 4.4'] };
    default:
...
const KIND_LABEL_KEY: Record<StateGlyphKind, string> = {
  ...
  'not-downloaded': 'mindMap.notInLibrary',
  ...
  template: `
    ...
    >
      @if (shape().circle; as circle) {
        <circle cx="8" cy="8" r="6.25" [attr.stroke-dasharray]="circle.dashArray ?? null" />
      }
